 ...
 *
 * Reads JSON from stdin, tries the daemon, falls back to local risk classification.
 * Run via: bun run packages/hooks/src/user-prompt-submit.ts
 *   or:   node --experimental-strip-types packages/hooks/src/user-prompt-submit.ts
 *   or:   npx tsx packages/hooks/src/user-prompt-submit.ts
 */
...
import { fallbackUserPromptSubmit } from "./lib/fallback.js";
...
async function main(): Promise<void> {
  ...
  // 1. Try the daemon
  const daemonResult = await callDaemon("/hooks/user-prompt-submit", input);
  if (daemonResult?.hookSpecificOutput) {
    respond(daemonResult);
    return;
  }

  // 2. Fall back to local risk classification
  const fallbackResult = fallbackUserPromptSubmit(input);
  if (fallbackResult) {
