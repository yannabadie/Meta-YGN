...
- **System 2 async post-processing**: graph auto-population (Task/Evidence/Decision/Lesson nodes), entropy tracking, and heuristic evolution run in `tokio::spawn` after hooks return — zero latency impact
- **Heuristic evolution wire-up**: `SessionOutcome` recorded at session end, `evolver.evaluate_all()` triggered after 5 outcomes, best version persisted to SQLite
- **Topology run_plan()**: stop hook uses the `ExecutionPlan` generated by `TopologyPlanner` instead of hardcoded `run_range(8,12)` — Single topology skips stages, Horizontal double-verifies
- **Entropy + Plasticity in decide stage**: `overconfidence_score > 0.3` forces Revise (EGPO), `plasticity_lost` forces Escalate (RL2F)
- **Semantic CompactStage**: lesson clustering by Jaccard word overlap replaces string dedup, max 10 clusters
- **E2E integration test harness**: 7 scenarios validating full session lifecycle, guard blocking, session persistence, error tracking, heuristic recording, health check, fatigue tracking

### Fixed
- Rust/TS HookInput contract aligned: added `source`, `reason`, `trigger` fields to TypeScript schema
- README rewritten for honesty: "What Works Today" vs "Experimental Features" sections

### Changed
- Hooks use `run_plan()` with stored topology instead of hardcoded stage ranges
- All hook handlers access `SessionStore` for cross-hook state
- Graph memory auto-populated during normal operation (was HTTP-only)
